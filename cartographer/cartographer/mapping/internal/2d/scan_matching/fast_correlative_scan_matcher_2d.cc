...
bool FastCorrelativeScanMatcher2D::Match(
  ...
  auto val = MatchWithSearchParameters(search_parameters, initial_pose_estimate,
...
bool FastCorrelativeScanMatcher2D::MatchFullSubmap(
  ...
  auto val = MatchWithSearchParameters(search_parameters, center, point_cloud,
...
//计算最低分辨率下的所有解的得分，同时从大到小排列
  const std::vector<Candidate2D> lowest_resolution_candidates =
      ComputeLowestResolutionCandidates(discrete_scans, search_parameters);

//每一次都初始化score
  _score = 0;
//通过分支定界搜索，将结果保存在best_candidate中
...
FastCorrelativeScanMatcher2D::GenerateLowestResolutionCandidates(
  ...
  //将旋转的scan平移，形成所有candidates，然后传回去
  for (int scan_index = 0; scan_index != search_parameters.num_scans;
       ++scan_index) {
...
Candidate2D FastCorrelativeScanMatcher2D::BranchAndBound(
  ...
  if (candidate_depth == 0) {
    ...
    //深度为0，找到最底层
    //只有这里才有效
    _score = (_score > candidates.begin()->score ? _score : candidates.begin()->score);
