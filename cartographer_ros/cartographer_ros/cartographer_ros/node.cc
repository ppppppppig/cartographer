...
int Node::AddTrajectory(const TrajectoryOptions& options,
  ...
  //通过这个启动一个新的建图方法
      expected_sensor_ids = ComputeExpectedSensorIds(options, topics);//将topic映射为id
  const int trajectory_id =
      map_builder_bridge_.AddTrajectory(expected_sensor_ids, options);//根据这些id去建图
