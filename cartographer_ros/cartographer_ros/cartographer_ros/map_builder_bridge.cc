...
void MapBuilderBridge::HandleSubmapQuery(
  ...
  //通过trajectory_id和submap_index查询对应的submap，将
...
void MapBuilderBridge::OnLocalSlamResult(
  ...
  cartographer::common::MutexLocker lock(&mutex_);
  trajectory_state_data_[trajectory_id] = std::move(local_slam_data);//将local_slam_data的数据保存到trajectory_state_data中


//接下来的问题在于，如何知道现在在边缘位置                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    
  // static int a = 0;
  // while(((a++) % 5) == 0){
  //   LOG(ERROR) << local_pose.translation() << " ";
